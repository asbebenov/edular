...
class OrgtechController extends Controller
{
	...
	$orgModel->mesto = $target;
        try {
            $inspect = ListTech::where('inv_num',$inv)->first();
            if($inspect) {
            ...
            switch (true) {
                case $e instanceof AlreadyAdded:
	...
	$id = $req->id;
	//$listOrg = Otdel::find($id)->searchOrg;
